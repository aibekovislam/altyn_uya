...
export default function Questions({ questionRef }: any) {
    ...
    const questionsWithImages = questions.map((question: any, index: number) => ({
        ...question,
        image: index === 0 ? GoatIMG.src : index === 1 ? CowIMG.src : null,
        text: index === 0 ? "Алтын Уя” Коомдук Фондунун Qurban.org сайты аркылуу бир күндүн ичинде курмандык чалдыра аласыз." : index === 1 ? "Себептери жана сооптору:" : index === 2 ? "КУРМАНДЫКТЫН КАНДАЙ ТҮРЛӨРҮН СОЙДУРА АЛАМ?" : null,
